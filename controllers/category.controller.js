...
export async function getSubCategoriesByParent(request, response) {
    try {
        ...
        const subCategories = await CategoryModel.find({ parentId }).sort({ name: 1 });
        return response.status(200).json({
            error: false,
            success: true,
            data: subCategories
...
// Get third-level categories by parent subcategory ID
export async function getThirdLevelByParent(request, response) {
    try {
        const { parentId } = request.params;
        if (!parentId) {
            return response.status(400).json({
                message: "Parent subcategory ID is required",
                error: true,
                success: false
            });
        }
        const thirdLevel = await CategoryModel.find({ parentId }).sort({ name: 1 });
        return response.status(200).json({
            error: false,
            success: true,
            data: thirdLevel
...
//get category count - getCategoriesCount
...
//get sub category count - getSubCategoriesCount
...
//get single category - getCategory
...
//removeImageFromCloudinary - removeImageFromCloudinary
