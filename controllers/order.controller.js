...
export const createOrderZinnaController = async (request, response) => {
    try {
        const {
            userId,
            totalAmount,
        ...
        if (!userId || !totalAmount || !delivery_address || !Array.isArray(products) || products.length === 0) {
            return response.status(400).json({
                error: true,
                success: false,
                message: "Missing required fields for Zinna checkout.",
            });
        }

        const zinnaApiBase = process.env.ZINNA_API_BASE_URL || "https://api.zinna.ae";
        const zinnaApiKey = process.env.ZINNA_API_KEY || "QyOJOF+BUy2O6izNOl1nc1euSJw/I9hcNROeOfk814p4qWhwRmNWvJ90AdIvWnWR";
        const zinnaApiSecret = process.env.ZINNA_API_SECRET || "";
        ...
        const zinnaResponse = await axios.post(
            `${zinnaApiBase.replace(/\/+$/, "")}/payments`,
            payload,
            {
                headers: {
                    Authorization: `Bearer ${zinnaApiKey}`,
                    ...(zinnaApiSecret ? { "X-Api-Secret": zinnaApiSecret } : {}),
        ...
        return response.status(200).json({
            success: true,
            error: false,
            checkoutUrl: zinnaResponse?.data?.checkoutUrl || zinnaResponse?.data?.paymentUrl || zinnaResponse?.data?.url || "",
            data: zinnaResponse?.data,
        });
    } catch (error) {
        return response.status(500).json({
